...
class BaseGenerator:
    ...
    def __get_symbol_cost(self, symbol, seen=set()):
        ...
        if symbol not in self.symbol_costs:
            self.symbol_costs[symbol] = min(
                self.__get_expansion_cost(p, seen | {symbol}) for p in self.expansions[symbol]
            )
    ...
    def __get_expansion_cost(self, expansion, seen=set()):
        ...
        if expansion not in self.expansion_costs:
            ...
            if len(symbols) == 0:
                ...
                self.expansion_costs[expansion] = sum(self.__get_symbol_cost(symbol, seen) for symbol in symbols) + 1
    ...
    def __expand_tree_by_phase(self, root, expand_node_method, limit=None):
        ...
        while ((limit is None) or (self.__possible_expansions(root) < limit)) and self.__able_to_expand(root):
    ...
    def __expand_node_by_cost(self, node, choose):
        ...
        possible_children_with_costs = [
            (self.__expansion_to_children(e), self.__get_expansion_cost(e, {symbol}), e) for e in expansions
        ]
    ...
    def __expand_tree_once(self, root):
        ...
        if children is None:
            ...
            possible_children_to_expand = [i for (i, c) in enumerate(children) if self.__able_to_expand(c)]
    ...
    def __expansion_to_children(self, expansion):
        ...
        if expansion not in self.__expanded_children:
            if expansion == "":
                ...
                self.__expanded_children[expansion] = [
                    (s, None if self.__is_non_terminal(s) else []) for s in all_strings
                ]
